...
def genetic_algorithm(initial_population, base_expanded, generations=100, survival_rate=0.5, output_dir=f'{IMAGE_FILE}_{time.time()}_genetic_frames'):
    """
    GAを実行し、進化完了の判定を行う。
    ...
    for generation in range(generations):
        ...
        # 世代ごとに最良個体を画像として保存
        best_image = generate_image_from_deltas(best_individual, base_expanded)
        cv2.imwrite(f"{output_dir}/gen_{generation:03d}_fitness_{max_fitness:.2f}.jpg", best_image)
